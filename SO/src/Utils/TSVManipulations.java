	...
	public static FileManipulationResult saveKeyAndValuesAsTSVFile(String outputPathAndFileName, TreeMap<String, Long> counts, 
		...
		try{
			...
			for (int j=1; j<totalFieldsCount; j++)
				writer.append(Constants.SEPARATOR_FOR_FIELDS_IN_TSV_FILE + titles[j]);
			writer.append("\n");
						...
						for (int j=0; j<fields1.length; j++)
							if (neededFieldNumbers1.contains(j))
								if (newGeneratedLine.equals(""))
									...
									newGeneratedLine = newGeneratedLine + Constants.SEPARATOR_FOR_FIELDS_IN_TSV_FILE + substituteForNullValuesInJoin;
	...
	public static FileManipulationResult joinSeveralTSVsWithIdenticalIDs(String iOPath, ArrayList<String> inputTSVs, String outputTSVFileName, 
		...
		try{ 
			...
			for (int p=0; p<inputTSVs.size(); p++){
				...
				String[] titlesOfAFile = s.get(p).split(Constants.SEPARATOR_FOR_FIELDS_IN_TSV_FILE);
...
//			ArrayList<String[]> fields = new ArrayList<String[]>();
			...
			while ((result.errors==0) && (!anyOfTheLinesIsEmpty)){//: s is initialized with the titles before entering the loop.
				...
				for (int p=0; p<inputTSVs.size(); p++){
					String[] fields = s.get(p).split(Constants.SEPARATOR_FOR_FIELDS_IN_TSV_FILE);
					for (int j=0; j<fields.length; j++)
						if (neededFieldNumbers.get(p).contains(j))
							if (newGeneratedLine.equals(""))
								newGeneratedLine = fields[j];
							else
								newGeneratedLine = newGeneratedLine + Constants.SEPARATOR_FOR_FIELDS_IN_TSV_FILE + fields[j];
					if (p==0)
